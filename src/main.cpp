...
uint256 hashGenesisBlock("0xbcf0582567734736c08f9416096841155fa0447bc521dd1ab5eefcb4cba1cfd3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 7: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "7 Signed Message:\n";
...
{
    int64 nSubsidy = 0.000007 * COIN;
    if(nHeight == 7)
    {
       nSubsidy = 0.000049 * COIN;
    }else if(nHeight == 77) // yay its 77 :)
    {
       nSubsidy = 0.000049 * COIN;
    ...
    {
       nSubsidy = 0.000049 * COIN;
    }else if(nHeight == 7777777)
    {
       nSubsidy = 0.000049 * COIN;
    }else if(nHeight == 77777777)
    {
       nSubsidy = 0.000049 * COIN;
    }
...
static const int64 nTargetTimespan = 420;	// 7: 7 minutes
static const int64 nTargetSpacing = 77;						// 7: 77 sec
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 588; // 14 Minutes
static const int64 nTargetSpacingRe = 77; // 7: 77 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
   ...
   if(fNewDifficultyProtocol) {
    ...
    // 7: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal 7 miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "7", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfe6dd4ae27e3ae32acc837ee4aa0fd3323d56fdfd3ae1191440730ef4188bae3");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "My hands are typing codez";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390505593;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1153626;

        if (fTestNet)
        {
            block.nTime    = 1390503934;
            block.nNonce   = 312547;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x256e5b736b21f6763147e42b7e7cd4a4673b092a200c1bda9dcee6a6e44c0fee"));
...
{
	return false;
    //commenting out this code until we properly implement 7 alerts.
	/*
    if (!CheckSignature())
        return false;
    if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // 7: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // 7: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
